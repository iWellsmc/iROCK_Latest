     ...
     data: null,
       render: function (data, type, row) {
         ...
         $(data.instruction_number).each(function(ind,val){
            actions_html+='<div class="get_paid_invcost" style="display: none;">'+val["cost_number"]+'<input type="hidden" id="inv_cost_id" value="'+val["cost_id"]+'"><input type="hidden" id="inv_cost_split_count" value="'+val["payment_split"]+'"></div>'
            paid_count++
          })
         ...
         render: function (data, type, row) {
            ...
            // actions_html+='<a class="btn px-1" href="/invoice/getinvoicesplit/'+data.id+'"><button type="button" class="btn btn-clr">View Invoice</button></a>'
                    ...
                    if (data.invoiceapproval_dispute > 0){
                        ...
                        if(data.dispute_query_status>0){
                            actions_html+='<a class="btn his-oty px-1" href="/invoice/invoicequeryhistory/'+data.id+'"><i class="fa fa-history" title="History" aria-hidden="true"></i></a>'
                        }
                    }
                    if(payment_type != 3){
                            if(payment_type != 3){
                                $(data.checkpermission_bank).each(function(ind,val){
                                    actions_html+='<a class="btn px-1 check_sign all-icon-same-clr" href="/invoice/bankuserview/'+data.id+'/'+val+'" data_id="'+data.id+'"><button type="button" class="btn btn-clr invoice-pay-btn">Confirmation for Payment</button></a>'
                                })
                            ...
                            if (data.module7_override > 0){
                                ...
                                $(data.payment_recept).each(function(ind,val){
                                    if (val!=null){
                                    actions_html+='<a class="btn px-1 pay_rec_cls check_sign inv_app_cls file_uploading upload'+data.id+'"  pay_id="'+val+'"  data-toggle="modal" id="file_uploading" data-target="#exampleModalCenter" data_id="'+data.id+'"><i class="fa-solid fa-receipt" style="color: #95183a; font-size: 17px;" title="Payment Receipt Confirmation"></i></a>'
                                    }
...
$(document).on('click','.file_uploading',function(){
    ...
    if (val > 0){
        ...
        if (file.length>0){
            ...
            test_val.each(function(){
                let pay_count=$(this).find('#inv_cost_split_count').val()
                if(pay_count == pay_id){
            ...
            $(new Array(len)).each(function(key,value){
                $.ajax({
                    url: `/invoice/paysplitpercentage/${inv_id}/${pay_id}/${li[key]}/`, 
                    type: 'GET', 
                    success: function(response) {
                        // Handle a successful response
                        $('.modal-body').append(`<table class="upload-choose-file">
                        <tbody class="val_details"><tr><td class="content">${li[key]}</td></tr>
                        
                        <tr class="flex_val"><td><input type="text" name="payment_percentage_${inv_cost_list[key]}" value="${response.percentage}%" placeholder="%" class="form-control payment-precentage">
                        <input type="hidden" name="get_paymentcount_${inv_cost_list[key]}" value="${pay_id}"></td><td><input type="file" accept="image, .png, .jpeg, .pdf, .jpg" name="file_${inv_cost_list[key]}[]" class="form-control filecls inp-voice3 filesize"><span class="file-error-message"></span></td><td><button class="add" data_id=${inv_cost_list[key]}><i class="fa fa-plus"></i></button></td></tr>
                        <tr><td></td><td><span class="maxmp">(Max: 25MB)</span></td></tr>
                        </tbody></table>`)
                    },
    ...
    $(document).on('click','.check_sign',function(event){
        ...
        if (val > 0){
            ...
            Swal.fire({
                text: 'Please add signature in User Details',
                confirmButtonText: 'OK'
            }).then((result) => {
                if (result.isConfirmed) {
                    var currentLocation = window.location.origin;
                    window.location.href = currentLocation +'/userdetails/'+user_id;
...
$(document).on('click','.add',function(event){
    ...
    event.preventDefault();
    $(`<tr class="flex_val"><td></td><td><input type="file" accept="image, .png, .jpeg, .pdf, .jpg" name="file_${data_id}[]" class="form-control filecls inp-voice3 filesize"><span class="file-error-message"></span></td><td><button class="delete"><i class="fa fa-minus"></i></button></td></tr>`).insertAfter($(this).closest('tr')) 
})
...
/* For Filter option */
    ...
    $(document).ready(function() {
      ...
      $(document).on('change', '#vendorvin, #vendorname ,#servicetype ,#paid', function () {
        var vid_id = $('#vendorvin').val().toLowerCase();
        var vendor_id = $('#vendorname').val().toLowerCase();
        var service_id = $('#servicetype').val().toLowerCase();
        var paid_id = $('#paid').val().toLowerCase();
        ...
        $("#tabledata tr").filter(function () {
            var row = $(this);
            var rowVId = row.find('.vid-id-column').text().toLowerCase();
            var rowProjectId = row.find('.vendor-id-column').text().toLowerCase();
            var rowService = row.find('.service-id-column').text().toLowerCase();
            var rowPaid = row.find('.paid-id-column').text().toLowerCase();
     
            var vinCondition = rowVId.indexOf(vid_id) > -1;
            var ServiceCondition = rowService.indexOf(service_id) > -1;
            var vendorCondition = rowProjectId.indexOf(vendor_id) > -1;
            var paidCondition = rowPaid.indexOf(paid_id) > -1;

            console.log(vendorCondition);
            // Toggle the row based on both conditions
            row.toggle(vinCondition && vendorCondition && ServiceCondition && paidCondition );
        });
    });

...
jQuery(document).ready(function(){
    var id=$('#gettype').attr('dataid')
    if ($('#gettype').val() == ''){
        swal.fire({
            text: "Company organization not select",
            type: "success"
            }).then(function() {
        ...
        onStepChanging: function (event, currentIndex, newIndex) {
           ...
           checkboxName: function(option) {
               return 'multiselect[]';
                ...
                //     // data: [{ id: 'all', text: 'All' }, /* other options */],
                // }).on('select2:selecting', function(e) {
                //     // if (e.params.args.data.id === 'all') {
                //     //     $(this).find('option[id="all"]').prop('selected', false);
                //     //      // Select all other options
                //     //      $(this).find('option:not([value="all"])').prop('selected', true);
                //     //     // Trigger change event to reflect the changes
                //     //     $(this).trigger('change');
                //     //     $('.select2-results__option').each(function() {
                //     //         $(this).attr('aria-selected', 'true');
                //     //     });
                //     // } else {
                //         console.log(e.params.args.originalEvent.currentTarget);
                ...
                //     // } else {
                //             var cur = e.params.args.data.id;
                //             var old = $(e.target).val().filter(function(e) {
                //             return e != cur;
                //             });
                //             $(e.target).val(old).trigger('change');
...
$(document).ready(function(){
    $(document).on('click', '.final-cls', function() {
        ...
        $('.intro').hide();
        $('.pc-draft-cls').hide();
        // $('.pc-draft-cls').prop("disabled",true);
    });
});
...
// })
    ...
    $(document).on('change', '#project_id', function() {
        ...
        $.ajax({
                ...
                {
                    console.log(data)
                    var html='<label for="staticEmail" class="col-6 col-form-label-project">Block Name</label><br>'
                    $.each(data.projects,function (key,val) {
                    ...
                    $('.Default_sign').attr('data-id',project_id)
                } 
        })
    })
    $(document).on('change', '.master_blk_cls', function() {
        ...
        $.ajax({
                ...
                success: function(data)
                {
                    ...
                    var html=''
                    html +='<div id=master_block-'+block_id+' class="master-block-field"><label class="col-6 col-form-label-project">Field Name</label><br>'
...
$(document).on('change','.field_blk_cls',function(){
    ...
    $.ajax({
        ...
        success: function(data)
        {
            console.log(data.data);
            var html=''
            html +='<div id=environment-'+field_id+' class="environement-cls "><label class="col-6 col-form-label-project">Field Environment</label><br>'
            $.each(data.data,function(key,val){
...
$(document).on('click','.pc-draft-cls',function(){
    var form = $("#project_create")
    form.submit();
})
